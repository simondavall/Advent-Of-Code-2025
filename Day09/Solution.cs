...
internal static partial class Program
{
  ...
  {
    var tilesArr = GetTiles(data);
    var tiles = tilesArr.ToHashSet();
    var edges = CreateEdges(tilesArr);
  ...
  private static bool IsCompletelyContained((long x, long y) a, (long x, long y) b, HashSet<(long x, long y)> tiles, HashSet<(long x, long y)> edges)
  {
    if (a.y > b.y) (a.y, b.y) = (b.y, a.y);
    if (a.x > b.x) (a.x, b.x) = (b.x, a.x);
    ...
    var y = a.y + 1;
    for (var x = a.x + 1; x < b.x; x++) {
      if (tiles.Contains((x, y)) || edges.Contains((x, y))) {
        return false;
      }
    }
